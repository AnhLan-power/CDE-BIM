...
let carbonBatches = []; // [{ material, factor, elements: [{id,name,volume,estimated,carbon}], totalVolume, totalCarbon }]
...
window.toggleCarbonPanel = function () {
  ...
  if (willShow) { renderCarbonParamsBox(); renderCarbonSummary(); renderCarbonBatchList(); }
};
...
function renderCarbonParamsBox() {
  ...
  box.innerHTML = `
    <div class="cde-file-card">
      ...
      <div style="font-size:10px;color:#888;margin-top:4px;">Dùng "Chọn Nhiều" để chọn 1 nhóm cấu kiện CÙNG loại vật liệu (vd: chọn hết cột bê tông) rồi bấm — lặp lại cho từng nhóm vật liệu khác nhau.</div>
    </div>`;
...
// Tìm khối lượng (m³) của 1 cấu kiện từ Pset thật của IFC (ưu tiên
// NetVolume > GrossVolume/Volume). Trả về null nếu không tìm thấy.
...
async function runCarbonAnalysis() {
  ...
  carbonBatches.push({
    material: DEFAULT_CARBON_FACTORS[materialKey].label,
    factor, elements, totalVolume, totalCarbon
  });
...
function carbonValueToColor(carbon, maxCarbon) {
  ...
  return [0.25 + t * 0.65, 0.55 - t * 0.4, 0.2]; // xanh lá nhạt (thấp) -> cam đậm (cao)
}
...
function clearAllCarbonData() {
  if (!confirm("Xoá toàn bộ kết quả đã tính (không xoá dữ liệu model)?")) return;
...
function renderCarbonSummary() {
  ...
  box.innerHTML = `
    <div class="cde-file-card">
      ...
      <button class="btn" style="width:100%;margin-top:6px;" id="cbClearAllBtn">🗑 Xoá toàn bộ kết quả</button>
    </div>`;
...
function renderCarbonBatchList() {
  ...
  carbonBatches.forEach((b, idx) => {
    ...
    card.innerHTML = `
      ...
      <div class="meta">
        ${b.totalVolume.toFixed(2)} m³ · ${formatCarbon(b.totalCarbon)} CO2e
